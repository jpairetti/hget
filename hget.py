...
def dns_resolve(hostname: str) -> str:
    ...
    # 1. Generar ticket y construcción de mensaje de consulta
    query_id = random.randint(0, 65535)
    pack = _dns_build_query(hostname, query_id)

    # 2. Crear socket UDP y configurar cronómetro
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.settimeout(5.0)

    try:
        # 3. Envío de mensaje
        sock.sendto(pack, (DNS_SERVER, DNS_PORT))

        # 4.1: Espero activamente la Rta.
        data, addr = sock.recvfrom(512)
        return _dns_parse_response(data, query_id)

    except socket.timeout:
        # 4.2: Error si paso el tiempo.
        print("Error: El servidor DNS no respondió (Timeout)", file=sys.stderr)
        raise ConnectionError("Fallo en la resolución DNS") from None
...
def connect_to_server(server_name: str, port: int = HTTP_PORT) -> socket.socket:
    ...
    # 1. Información en stderr
    print(f"Hostname: {server_name}", file=sys.stderr)
    print(f"IP resuelta: {ip_address}", file=sys.stderr)

    # 2. Creación de socket TCP (SOCK_STREAM)
